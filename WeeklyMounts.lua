function MC.weeklyDisplay()
    ...
    difficultyNames = {
        [3] = "10P", [4] = "25P", [5] = "10P (H)", [6] = "25P (H)",
        [14] = "N", [15] = "H", [16] = "M", [17] = "LFR", [23] = "Mythic"
    },
                    ...
                    for _, bossData in ipairs(bosses) do
                        if type(bossData) == "table" then
                            ...
                            if #mountNames > 0 and MasterCollectorSV.showMountName then
                                mountsText = "\n          Mount: " .. table.concat(mountNames, ", ") .. rarityAttemptsText .. dropChanceText
                            end
                            if not skipBoss then
                                if aura_env.raidsWithoutDifficultyCheck[instanceID] then
                                    ...
                                    difficultiesText = difficultiesText:sub(1, -3)                                    
                                    if not (MasterCollectorSV.showBossesWithNoLockout and allDifficultiesKilled) then
                                        ...
                                        instanceText = instanceText .. "      - " .. bossColor .. bossName .. " (" .. difficultiesText .. ")|r" .. mountsText .. "\n"
                                    end
                                    ...
                                    for _, difficulty in ipairs(difficulties) do
                                        ...
                                        if difficulty == 17 then
                                            ...
                                            if not (MasterCollectorSV.showBossesWithNoLockout and isKilled) then
                                                instanceText = instanceText .. "      - " .. color .. bossName .. " (" .. difficultyName .. " - " .. wingName .. ")|r" .. mountsText .. "\n"
                                            end
                                        else
                                            if not (MasterCollectorSV.showBossesWithNoLockout and isKilled) then
                                                instanceText = instanceText .. "      - " .. color .. bossName .. " (" .. difficultyName .. ")|r" .. mountsText .. "\n"
                                            end
                                        end
                                    end
                                end
            ...
            for _, bossData in ipairs(aura_env.dungeonData["World Boss Mounts"][category.name]) do
                if type(bossData) == "table" then
                    ...
                    if not skipBoss then
                        ...
                        local isKilled = isWorldBossKilled(bossID)
                        local showBoss = true
                        ...
                        if showBoss then
                            ...
                            if not (MasterCollectorSV.showBossesWithNoLockout and isKilled) then
                                subtitleText = subtitleText .. "      - " .. color .. bossName .. "|r" .. mountsText .. "\n"
                            end
                        end
                    end
                end
    ...
    local function GarrisonInvasionMounts()
        ...
        for _, mountData in ipairs(garrisonInvasionData) do
            ...
            if not (MasterCollectorSV.showBossesWithNoLockout and questCompleted) then
                ...
                local mountsText = ""
                if #mountNames > 0 and MasterCollectorSV.showMountName then
                    mountsText = "          Mount: " .. table.concat(mountNames, ", ") .. rarityAttemptsText .. dropChanceText
                end
                if mountsText ~= "" then
